...
client.on('guildCreate', async guild => {
  var guildhook = new Discord.WebhookClient("640146601653829644", "Qd0nSqY7UL7QYU3JjALbiEoN-eiZbH9lp-D4GuJm9XDjyltAQDHN8YdWDrQehGyd115L")
...
if (veri === true) {
  ...
  client.on('message', msg => {
    ...
    if (regex.test(msg.content)==true) {
      if (!msg.member.hasPermission("ADMINISTRATOR")) {
        ...
        return msg.reply('Bu sunucuda linkler **Marmara Bot** tarafından engellenmektedir.Link atmana izin vermeyeceğim!').then(msg => msg.delete(3000));
      } else {
        return;
      };
    } else {
  ...
  client.on('message', msg => {
    if (msg.content.toLowerCase() === '<@457547769159352341>') {
      msg.reply('Komutlarımı öğrenmek için; \n `@Marmara Bot#4077 yardım` veya \n `>yardım` yazabilir, \n Beni sunucuna eklemek ve Destek Sunucum\'a gelmek için ise `@Marmara Bot#4077 davet` veya `>davet` yazabilirsin.');
    }
  });
...
client.on('ready', () => {
  ...
  client.setInterval(() => {
      client.user.setActivity("MarmaraNetwork !yardım", { type: "WATCHING" });
  }, 15000);
});
